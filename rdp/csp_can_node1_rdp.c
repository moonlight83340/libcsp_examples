...
int main(void) {
	...
	csp_print("Initializing CSP with CAN device: %s\n", VCAN_DEVICE);
	csp_init();
	router_start();

	csp_iface_t * default_iface = NULL;
	if (csp_can_socketcan_open_and_add_interface(VCAN_DEVICE, CSP_IF_CAN_DEFAULT_NAME, CLIENT_ADDRESS, 1000000, true, &default_iface) != CSP_ERR_NONE) {
	...
	while (1) {
		...
		/* Instead of using no option (CSP_O_NONE) here we use RDP option (CSP_O_RDP) */
		csp_conn_t * conn = csp_connect(CSP_PRIO_NORM, SERVER_ADDRESS, SERVER_PORT, 1000, CSP_O_RDP);
		if (!conn) {
